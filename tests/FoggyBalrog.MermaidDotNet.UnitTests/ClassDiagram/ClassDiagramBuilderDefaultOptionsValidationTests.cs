...
public class ClassDiagramBuilderDefaultOptionsValidationTests
{
    [Fact]
    ...
    public void AddHyperlink_ThrowsIfWhiteSpaceUri()
    {
        ...
        Assert.Equal(MermaidExceptionReason.InvalidUri, exception.Reason);
    }

    [Fact]
    public void AddHyperlink_ThrowsIfInvalidUri()
    {
        var exception = Assert.Throws<MermaidException>(() =>
        {
            Mermaid
                .ClassDiagram()
                .AddClass("A", out var a)
                .AddHyperlink(a, "http://example.com\\foo", "bar");
        });
